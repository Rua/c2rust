...
impl<'c> Translation<'c> {
    pub fn convert_address_of(
        ...
        let val = self.convert_expr(ctx.set_needs_address(true), arg, None)?;
    ...
    pub fn convert_deref(
        ...
        self.convert_expr(ctx.set_needs_address(false), arg, None)?
    ...
    pub fn convert_array_subscript(
        ...
        if let Some(array_id) = simple_index_array {
            ...
            let array_rs = self.convert_expr(ctx.set_needs_address(false), array_id, None)?;
            ...
            let pointer_rs =
                self.convert_expr(ctx.set_needs_address(false).decay_ref(), pointer_id, None)?;
            let target_type_id = self.ast_context.type_for_kind(&CTypeKind::SSize);
            let offset_rs = self.convert_expr_with_cast(
                ctx.set_needs_address(false),
                CQualTypeId::new(target_type_id),
